...
function renderIssueButtons() {
    ...
    issues.forEach(issueFolder => {
        ...
        btn.onclick = (e) => selectIssue(issueFolder, e.target);
        container.appendChild(btn);
    });

    if (issues.length > 0) {
        const firstBtn = container.querySelector('.issue-btn');
        selectIssue(issues[0], firstBtn);
    }
}

// Select an issue
function selectIssue(issueFolder, btnElement) {
...
function renderModelButtons() {
    ...
    models.forEach(model => {
        ...
        btn.onclick = (e) => selectModel(model, e.target);
        container.appendChild(btn);
    });

    if (models.length > 0) {
        const firstBtn = container.querySelector('.model-btn');
        selectModel(models[0], firstBtn);
    }
}
